...
foreach ($rows as $feed) {
	...
	foreach ($content->get_items() as $item) {
		...
		if (count($itemrows) == 0) {
			...
			$insertquery =
				"INSERT INTO items (id,feedTitle,feedLink,itemPubDate,itemLink,itemDesc,display) VALUES (" .
				$feed['id'] . ",'" .
			...
			$insertquery =
				"INSERT INTO items (id,feedTitle,feedLink,itemTitle,itemPubDate,itemLink,itemDesc,display) VALUES (" .
				$feed['id'] . ",'" .
				$item->get_feed()->get_title() .
				"','" .
				$item->get_feed()->get_permalink() .
